...
const signal = wins.map(w => {
  const d = contextDistance(wins, w.tMid, norms, CTX) || {};
  return { t: w.tMid, Dm: d.D_motion ?? null, Dp: d.D_pose ?? null, comb: d.combined ?? null };
});
const at = autothreshold(signal);
const high = at ? at.high : 1.0, low = at ? at.low : 0.3;
const cands = detectCandidates(signal, high, low, 0.7);
const segments = segmentsFromCandidates(cands, dur);

const valid = frames.filter(f => f.desc).length;
console.log(`${name}: dur=${dur.toFixed(1)} с · кадров ${frames.length} (валид ${valid}) · окон ${wins.length}`);
console.log(`автопорог: high=${high.toFixed(3)} low=${low.toFixed(3)}`);
const vals = signal.filter(s => s.comb != null);
if (vals.length){
  const mx = vals.reduce((a, b) => a.comb > b.comb ? a : b);
  console.log(`сигнал: ${vals.length} точек, max=${mx.comb.toFixed(2)} на t=${mx.t.toFixed(1)} с`);
} else {
  console.log("сигнал пуст — нечего сегментировать");
}
console.log(`кандидаты: ${cands.length}`);
...
const out = {
  settings: { win: WIN, step: STEP, ctx: CTX, rate: json.rate_hz || 5,
              high: +high.toFixed(3), low: +low.toFixed(3), frac: 0.7, iou: 0.3 },
