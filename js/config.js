...
// Ниже этого порога форма точно «не та» — зануляем фичу даже при малых углах
// (иначе случайно близкие углы чужого движения спасаются исключением abs<=thr*0.5).
export const SYNC_HARD = 0.6;
// Плотность сэмплов (сэмпл/с), ниже которой корреляция формы ненадёжна (алиасинг
// быстрых фич при разрежённом детекте камеры) — там гейтом не нулим.
export const SYNC_REL_MIN_SPS = 4;
...
// ── σ_noise (Фаза 1): робастная оценка шума угла фичи на этом устройстве ──
// Остатки val − median3 по коротким окнам B (≤ SIGMA_WIN_S — кривизна движения
// мизерна, остаток шум-доминирован) → MAD/0.6745, EMA + заморозка.
// Фолбэк, если остатков набралось меньше SIGMA_MIN_RESID к моменту заморозки.
export const DEFAULT_SIGMA = 3;
// Макс. длительность окна B (с) для остатка: дальше — движение, не шум.
export const SIGMA_WIN_S = 0.25;
// Сколько остатков нужно для доверительной оценки σ.
export const SIGMA_MIN_RESID = 30;
// Заморозка после стольких секунд сессии (шумы условий дальше не учитываем).
export const SIGMA_FREEZE_S = 6;
// Скачок B больше этой величины — спайк/телепорт, остаток не копим.
export const SIGMA_SPIKE = 25;
